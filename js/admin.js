jQuery(document).ready(function($){
    ...
    // Properties|Agents|Agency Template
	$(document).on('change', '#page_template', function(){
        var val = $(this).val();
        show_hide_property_setting(val);
    });
    setTimeout(function() {
    ...
    function show_hide_property_setting(val) {
        if ( val == 'page-properties.php' ) {
            $('#apus_page_properties_setting').show();

            //$('.cmb2-id-apus-page-layout').hide();
            $('.cmb2-id-apus-page-left-sidebar').hide();
            $('.cmb2-id-apus-page-right-sidebar').hide();
            if ( !$('#apus_page_layout_type').val() || $('#apus_page_layout_type').val() == 'default' || $('#apus_page_layout_type').val() == 'top-map' ) {
                $('.cmb2-id-apus-page-layout').show();
            } else {
                $('.cmb2-id-apus-page-layout').hide();
            }
            if ( !$('#apus_page_properties_show_filter_top').val() || $('#apus_page_properties_show_filter_top').val() == 'yes' ) {
                $('.cmb2-id-apus-page-properties-filter-top-sidebar').show();
            } else {
                $('.cmb2-id-apus-page-properties-filter-top-sidebar').hide();
            }

            if ( $('#apus_page_display_mode').val() == 'grid' ) {
                $('.cmb2-id-apus-page-inner-grid-style').show();
                $('.cmb2-id-apus-page-inner-list-style').hide();
            } else if ( $('#apus_page_display_mode').val() == 'list' ) {
                $('.cmb2-id-apus-page-inner-list-style').show();
            ...
            $('#apus_page_layout_type').on('change', function(){
                if ( !$(this).val() || $(this).val() == 'default' || $(this).val() == 'top-map' ) {
                    $('.cmb2-id-apus-page-layout').show();
                } else {
                    $('.cmb2-id-apus-page-layout').hide();
                }
            ...
            $('#apus_page_properties_show_filter_top').on('change', function(){
            ...
            $('#apus_page_display_mode').on('change', function(){
                if ( $(this).val() == 'grid' ) {
                    $('.cmb2-id-apus-page-inner-grid-style').show();
                    $('.cmb2-id-apus-page-inner-list-style').hide();
            ...
            $('#apus_page_properties_setting').hide();
        ...
        // Agents template
        if ( val == 'page-agents.php' ) {
            $('#apus_page_agents_setting').show();
        } else {
            $('#apus_page_agents_setting').hide();
        }
        // Agencies template
        if ( val == 'page-agencies.php' ) {
            $('#apus_page_agencies_setting').show();
        } else {
            $('#apus_page_agencies_setting').hide();
        }
    }
    
    
    // loading admin icon
    $("body").on("wp_realestate_all_loading_icons", function(){
        justhome_get_awesome_icons();
        justhome_get_flaticon_icons();
        justhome_get_themify_icons();

        if ( $('#apus_tax_icon_font').length ) {
    ...
    function justhome_get_awesome_icons() {
        ...
        wp_realestate_all_loaded_icons.fontwesome = icons_load_call;
    }

    function justhome_get_flaticon_icons() {
        ...
        for( var i=0; i < files.length; i++ ){
            css = Array.prototype.map.call( files[i].sheet.cssRules, justhome_css_text ).join('\n');
        ...
        wp_realestate_all_loaded_icons.flaticon = icons_load_call;
    }

    function justhome_get_themify_icons() {
        ...
        wp_realestate_all_loaded_icons.themify = icons_load_call;
    }

    function justhome_css_text(x) {
        return x.cssText;
    }
});