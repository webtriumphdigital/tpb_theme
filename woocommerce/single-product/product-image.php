...
$nb_columns = justhome_get_config('number_product_thumbs', 7);
$post_thumbnail_id = get_post_thumbnail_id( $post->ID );

$thumbs_pos = justhome_get_config('product_thumbs_position', 'thumbnails-left');
...
<div class="apus-woocommerce-product-gallery-wrapper <?php echo esc_attr(($attachment_ids && has_post_thumbnail())?'':'full-width'); ?>">
      ...
      if (!empty($video)) {
        ...
        <div class="video">
          <a href="<?php echo esc_url($video); ?>" class="popup-video">
            ...
            <span class="text-theme"><?php echo esc_html__('Watch video', 'justhome'); ?></span>
          </a>
        </div>
        <?php
      }
	...
	<div class="slick-carousel apus-woocommerce-product-gallery" data-carousel="slick" data-items="1" data-large="1" data-medium="1" data-small="1" data-smallest="1" data-pagination="false" data-nav="false" data-slickparent="true">
		...
		if ( has_post_thumbnail() ) {
			...
			$html .= sprintf( '<img src="%s" alt="%s" class="wp-post-image" />', esc_url( wc_placeholder_img_src() ), esc_attr__( 'Awaiting product image', 'justhome' ) );
			$html .= '</div>';
		}
...
<?php if ( $attachment_ids && has_post_thumbnail() ) { ?>
	<div class="wrapper-thumbs <?php echo esc_attr($count_thumbs <= $nb_columns ? '' : ''); ?>">
		<div class="slick-carousel apus-woocommerce-product-gallery-thumbs <?php echo esc_attr($thumbs_pos == 'thumbnails-left' || $thumbs_pos == 'thumbnails-right' ? 'vertical' : ''); ?>" data-carousel="slick" data-items="<?php echo esc_attr($nb_columns); ?>" data-large="4" data-medium="6" data-small="5" data-smallest="3" data-pagination="false" data-nav="true" data-asnavfor=".apus-woocommerce-product-gallery" data-slidestoscroll="1" data-focusonselect="true" <?php echo trim($thumbs_pos == 'thumbnails-left' || $thumbs_pos == 'thumbnails-right' ? 'data-vertical="true"' : ''); ?>>
			<?php
			...
			echo apply_filters( 'justhome_woocommerce_single_product_image_thumbnail_html', $html, get_post_thumbnail_id( $post->ID ) );
			...
			foreach ( $attachment_ids as $attachment_id ) {
				...
				echo apply_filters( 'justhome_woocommerce_single_product_image_thumbnail_html', $html, $attachment_id );
			}
