...
function justhome_ocdi_import_files() {
    $demos = array();
    $demos[] = array(
        ...
        'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'justhome' ),
        'preview_url'                  => 'https://demoapus1.com/justhome/',
    );
    return apply_filters( 'justhome_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'justhome_ocdi_import_files' );

function justhome_ocdi_after_import_setup( $selected_import ) {
    // Assign menus to their locations.
    $main_menu       = get_term_by( 'name', 'Primary Menu', 'nav_menu' );
    $mobile_main_menu       = get_term_by( 'name', 'Primary Mobile Menu', 'nav_menu' );
    $agent_menu      = get_term_by( 'name', 'Agent Account Menu', 'nav_menu' );
    $agency_menu   = get_term_by( 'name', 'Agency Account Menu', 'nav_menu' );
    $user_menu   = get_term_by( 'name', 'User Account', 'nav_menu' );
    ...
    if ( file_exists($file) ) {
        justhome_ocdi_import_settings($file);
    }
}
add_action( 'pt-ocdi/after_import', 'justhome_ocdi_after_import_setup' );

function justhome_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        justhome_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        justhome_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function justhome_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function justhome_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function justhome_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'justhome_ocdi_before_widgets_import' );