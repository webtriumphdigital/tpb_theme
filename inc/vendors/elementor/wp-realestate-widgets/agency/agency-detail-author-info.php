...
class Justhome_Elementor_Widget_Detail_Agency_Author_Info extends Elementor\Widget_Base {

	public function get_name() {
		return 'apus_element_detail_agency_author_info';
	}

	public function get_title() {
		return esc_html__( 'Agency Details:: Author Info', 'justhome' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
            'show_license',
            [
                'label'         => esc_html__( 'Show License', 'justhome' ),
                'type'          => Elementor\Controls_Manager::SWITCHER,
                'label_on'      => esc_html__( 'Show', 'justhome' ),
                'label_off'     => esc_html__( 'Hide', 'justhome' ),
                'default'       => 'yes',
		...
		$this->add_control(
            'show_tax_number',
            [
                'label'         => esc_html__( 'Show Tax Number', 'justhome' ),
                'type'          => Elementor\Controls_Manager::SWITCHER,
                'label_on'      => esc_html__( 'Show', 'justhome' ),
                'label_off'     => esc_html__( 'Hide', 'justhome' ),
                'default'       => 'yes',
	...
	protected function render() {
		...
		if ( !empty($post) ) {
			...
			$location = justhome_agency_display_full_location($post, 'title', false);

			$phone = justhome_agency_display_phone($post, 'title', false, true);
			$fax = justhome_agency_display_meta_data($post, 'fax', esc_html__('Fax', 'justhome'), '', false);
			$email = justhome_agency_display_email($post, 'title', false);
			$website = justhome_agency_display_website($post, 'title', false);

			$license = justhome_agency_display_meta_data($post, 'license', esc_html__('License', 'justhome'), '', false);
			$tax_number = justhome_agency_display_meta_data($post, 'tax_number', esc_html__('Tax number', 'justhome'), '', false);

			$whatsapp = WP_RealEstate_Agency::get_post_meta( $post->ID, 'whatsapp' );

	        ?>
			<div class="agent-detail-element agency-detail-author-info <?php echo esc_attr($el_class); ?>">
				<?php if ( $show_avatar ) {
				...
				?>
					<div class="user-thumbnail img-agency">
						<?php justhome_agency_display_image($post, $thumbsize); ?>
					</div>
				<?php } ?>

				<div class="member-detail-user agency-detail-user">
					...
					<?php if ( $show_license && $license ) { ?>
						<?php echo trim($license); ?>
					<?php } ?>

					<?php if ( $show_tax_number && $tax_number ) { ?>
					...
					<?php if ( $show_social ) { ?>
						...
						<?php
				            $socials = get_post_meta( $post->ID, '_agency_socials', true );
...
Elementor\Plugin::instance()->widgets_manager->register( new Justhome_Elementor_Widget_Detail_Agency_Author_Info );
