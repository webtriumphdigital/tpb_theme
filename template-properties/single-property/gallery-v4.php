...
if ( has_post_thumbnail() || ($gallery && is_array($gallery)) ) {
    $gallery_size = !empty($gallery_size) ? $gallery_size : '1231x637';
    $gallery_second_size = !empty($gallery_second_size) ? $gallery_second_size : '160x120';

    $first_class = 'col-12';
    $second_class = 'col-md-12 col-3';
    if ( $gallery && is_array($gallery) ) {
        $first_class = 'col-md-10 c1 col-12';
    } else {
        $gallery_size = '1400x637';
    }
?>
<div class="property-detail-gallery v4">
    <div class="row row-10 wrapper">
        ...
        <?php if ( $gallery && is_array($gallery) ) { ?>
            <div class="col-md-2 c2 col-12 right-gallery">
                <div class="row row-10">
                    <?php $i=1; foreach ( $gallery as $id => $src ) {
                        ...
                        if ( $i == 5 && count($gallery) > 5 ) {
                            $more_image_html = '<div class="view-more-gallery d-none d-lg-block">'.sprintf(esc_html__('See All %s Photos', 'justhome'), count($gallery) - 2).'</div>';
