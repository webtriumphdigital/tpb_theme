...
$agent_inner_style = !empty($settings['agent_item_style']) ? $settings['agent_item_style'] : 'grid';
...
?>
<div class="agents-listing-wrapper main-items-wrapper" data-settings="<?php echo esc_attr(json_encode($settings)); ?>">
			...
			<div class="row">
				<?php while ( $agents->have_posts() ) : $agents->the_post(); ?>
					<div class="col-xl-<?php echo esc_attr($mdcol); ?> col-md-<?php echo esc_attr($smcol); ?> col-<?php echo esc_attr( $xscol ); ?>">
						<?php echo WP_RealEstate_Template_Loader::get_template_part( 'agents-styles/inner-'.$agent_inner_style ); ?>
					</div>
				<?php endwhile; ?>
			</div>
	...
	<?php } else { ?>
		<div class="not-found text-center"><?php esc_html_e('No agent found.', 'justhome'); ?></div>
	<?php } ?>
