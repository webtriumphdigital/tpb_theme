...
if ( !empty( $first_regions ) && !is_wp_error( $first_regions ) ) {
    ...
    if ( ! empty( $selected ) ) {
        if( !is_array($selected)) {
            if ( is_numeric( $selected ) ) {
                ...
                $term = get_term_by( 'term_id', $selected, 'property_location' );
    ...
    $placeholder = !empty($field['placeholder']) ? $field['placeholder'] : __('Filter by %s', 'justhome');
    ...
    <div class="form-group form-group-<?php echo esc_attr($key); ?> tax-select-field">
        ...
        ?>
            ...
            <div class="field-region field-region3">
                <select class="select-field-region select-field-region3 form-control" data-next="4" autocomplete="off" name="<?php echo esc_attr($name); ?>[]" data-placeholder="<?php echo esc_attr($placeholder3); ?>"  data-taxonomy="property_location">
