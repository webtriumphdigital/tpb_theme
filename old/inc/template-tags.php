<?php
/**
 * Custom template tags for Justhome
 *
...
if ( ! function_exists( 'justhome_comment_nav' ) ) {
	function justhome_comment_nav() {
		// Are there comments to navigate through?
		if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) :
		?>
		<nav class="navigation comment-navigation" role="navigation">
			<h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'justhome' ); ?></h2>
			<div class="nav-links">
				<?php
					if ( $prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'justhome' ) ) ) :
						printf( '<div class="nav-previous"><i class="fa fa-long-arrow-left" aria-hidden="true"></i> %s</div>', $prev_link );
					endif;

					if ( $next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'justhome' ) ) ) :
...
if ( ! function_exists( 'justhome_post_categories' ) ) {
	function justhome_post_categories( $post ) {
...
if ( ! function_exists( 'justhome_post_categories_first' ) ) {
	function justhome_post_categories_first( $post ) {
		$cat = wp_get_post_categories( $post->ID );
		if( sizeof($cat) != 0 ){
			echo '<div class="list-categories">';
			foreach ( $cat as $c ) {
...
if ( ! function_exists( 'justhome_post_thumbnail' ) ) {
	function justhome_post_thumbnail($thumbsize = '', $link = '') {
		...
		if ( is_singular('post') && is_single($post) ) {
			...
			$html .= justhome_get_attachment_thumbnail($post_thumbnail_id, 'full');
			$html .= '</div>';
			...
			$html .= '<figure class="entry-thumb">';
				$html .= '<a class="post-thumbnail" href="'.esc_url($link).'" aria-hidden="true">';
						$post_thumbnail_id = get_post_thumbnail_id();
						$html .= justhome_get_attachment_thumbnail($post_thumbnail_id, $thumbsize);
				$html .= '</a>';
...
if ( ! function_exists( 'justhome_display_post_thumb' ) ) {
	function justhome_display_post_thumb($thumbsize) {
		$post_format = get_post_format();
		$output = '';
		
        if ( has_post_thumbnail() ) {
            $output = justhome_post_thumbnail($thumbsize);
        }
	    return $output;
	}
}

function justhome_get_attachment_thumbnail($attachment_id, $size = 'thumbnail', $icon = false, $attr = '', $wrapper = true) {
	$html = '';
	...
	if ( $image ) {
		...
		if ( justhome_get_config('image_lazy_loading') ) {
			$src_layzy = justhome_create_placeholder(array($width, $height));
			$attr['data-src'] = $src;
			$attr['src'] = $src_layzy;
