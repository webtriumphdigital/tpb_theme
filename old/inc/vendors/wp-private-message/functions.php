...
//add_action( 'justhome_after_contact_form', 'justhome_private_message_form', 10, 2 );
function justhome_private_message_form($post, $user_id) {
	?>
	<div class="send-private-wrapper">
		<a href="javascript:void(0);" class="send-private-message-btn"><i class="fa fa-hand-o-right" aria-hidden="true"></i><?php esc_html_e('Send Private Message', 'justhome'); ?></a>
	</div>
	<div class="send-private-message-wrapper-hidden hidden">
		<div class="send-private-message-wrapper">
			<h3 class="title"><?php echo sprintf(esc_html__('Send message to "%s"', 'justhome'), $post->post_title); ?></h3>
			<?php
			if ( is_user_logged_in() ) {
				?>
				<form id="send-message-form" class="send-message-form" action="?" method="post">
	                <div class="form-group">
	                    <input type="text" class="form-control style2" name="subject" placeholder="<?php esc_attr_e( 'Subject', 'justhome' ); ?>" required="required">
	                </div><!-- /.form-group -->
	                <div class="form-group">
	                    <textarea class="form-control message style2" name="message" placeholder="<?php esc_attr_e( 'Enter text here...', 'justhome' ); ?>" required="required"></textarea>
	                </div><!-- /.form-group -->
	                ...
	                <button class="button btn btn-theme btn-block send-message-btn"><?php echo esc_html__( 'Send Message', 'justhome' ); ?></button>
	        	</form>
				<?php
			} else {
				$login_url = '';
				...
				?>
				<a href="<?php echo esc_url($login_url); ?>" class="login"><?php esc_html_e('Please login to send a private message', 'justhome'); ?></a>
				<?php
			}
			?>
		</div>
...
function justhome_private_message_user_avatar($user_id) {
	if ( class_exists('WP_RealEstate_User') && (WP_RealEstate_User::is_agency($user_id) || WP_RealEstate_User::is_agent($user_id)) ) {
	    if ( WP_RealEstate_User::is_agency($user_id) ) {
            ...
            $avatar = '<img src="'.esc_url($avatar_url).'" alt="'.esc_attr__('Image', 'justhome').'"/>';
	    } else {
